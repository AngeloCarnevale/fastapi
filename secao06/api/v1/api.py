...
async def post_artigo(artigo: ArtigoSchema, current_user: UsuarioModel = Depends(get_current_user), db: AsyncSession = Depends(get_session)):
...
async def put_artigo(artigo_id : int, artigo: ArtigoSchema, db: AsyncSession = Depends(get_session), current_user: UsuarioModel = Depends(get_current_user)):
    async with db as session:
        query = select(ArtigoModel).filter(ArtigoModel.id == artigo_id)
        result = await session.execute(query)
        ...
        if artigo_up:
            ...
            if current_user.id != artigo_up.usuario_id:
                artigo_up.usuario_id = current_user.id
